...
class Token
  ...
  def is_correct_weight
    @children.all?{|x| x.my_weight == @children[0].my_weight}
  end

  def find_anomaly
    grouped = @children.group_by{|child| child.my_weight}

    number_of_groups = grouped.count

    return nil if number_of_groups === 0

    if number_of_groups === 1 then
      return nil
    end

    if !grouped.select{|id, group| group.count === 1}.values[0][0].find_anomaly then
      p @name
      p self.my_weight
      @children.each {|c| p " - #{c.name} #{c.my_weight}"}
    end
  ...
  def my_weight
    @id + @children.inject(0){|sum, x| sum + x.my_weight }
  end
end

def get_token(name)
  return @nodes[name] if @nodes[name]
  @nodes[name] = Token.new(name)
...
def make_token(token_string)
  ...
  token = get_token(tokens[0].split(" ")[0])

  token.id = tokens[0][/\(([^)]+)\)/, 1].to_i
  children = tokens[1] ? tokens[1].split(",").map(&:strip) : []

  children.each do |child|
    token.add_child(get_token(child))
  end
...
root_node = @nodes.values.select { |node| node.parent.nil? }.first

anomaly = root_node.find_anomaly

rfkvap  = @nodes.values.select { |node| node.name === "rfkvap" }.first
p "-------"
#p anomaly.name

p rfkvap.id
rfkvap.children.each {|c| p c.my_weight}

#rfkvap - 646

#p anomaly.name
#anomaly.children.each {|c| p c.my_weight}
