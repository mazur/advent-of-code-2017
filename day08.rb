...
program.each do |row|
  tokens = row.split(" ")
  tokens[0] = 'reg["' + tokens[0] + '"]'
  tokens[4] = 'reg["' + tokens[4] + '"]'
  tokens[1] = tokens[1] == "inc" ? "+=" : "-="
  complied_program.push(tokens.join(" "))
end
...
class MemHash < Hash
  ...
  def []=(key,val)
    @highest_value = val if @highest_value < val
    super(key,val)
  end
end
...
reg = MemHash.new(0)
str_program = complied_program.join("\n");
